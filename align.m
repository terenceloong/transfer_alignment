...
switch N_a
    case 15
        P_a = diag([[1,1,1]*(2/180*pi), [1,1,1]*(2/180*pi), [1,1,1]*2, [1,1]*1e-6,5, [1,1,1]*(0.1/180*pi)].^2);
        Q_a = diag([[1,1,1]*sqrt(gyro_noise/dt), [1,1,1]*(0/180*pi), [1,1,1]*sqrt(acc_noise/dt),...
                    [8e-9,8e-9,0.05]*4, [1,1,1]*(80/3600/180*pi)].^2) *dt^2;
    case 18
        P_a = diag([[1,1,1]*(2/180*pi), [1,1,1]*(2/180*pi), [1,1,1]*2, [1,1]*1e-6,5, [1,1,1]*(0.1/180*pi), [1,1,1]*0.05].^2);
        Q_a = diag([[1,1,1]*sqrt(gyro_noise/dt), [1,1,1]*(0/180*pi), [1,1,1]*sqrt(acc_noise/dt),...
                    [8e-9,8e-9,0.05]*4, [1,1,1]*(80/3600/180*pi), [1,1,1]*0.000].^2) *dt^2;
end
R_a = diag([[1,1,1]*(0.1/180*pi), [1,1,1]*0.1, [1,1]*1.6e-7,1].^2);
...
%=========================================================================%
    ...
    if mod(k,5)~=0
        ...
        %---------measure update----------%
        Phi_a = state_align(avp, acc1, dt, N_a);
        [H_a, Z_a] = measure_align(avp, traj(k+1,:), qab, N_a);
        Z_a(4:6) = Z_a(4:6) + randn(3,1)*0.1;
        Z_a(7:8) = Z_a(7:8) + randn(2,1)*1.6e-7;
        Z_a(9) = Z_a(9) + randn(1)*1;
...
function [H, Z] = measure_align(avp, traj, qab, n)
    Cbn = angle2dcm(traj(7)/180*pi, traj(8)/180*pi, traj(9)/180*pi)';
    Cab = angle2dcm(-2/180*pi, 4/180*pi, 2/180*pi); %%%%%
    Cna = (Cbn*Cab)';
    qna = dcm2quat(Cna);
